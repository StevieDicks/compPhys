...
int main() {
  ...
  if((out = fopen("SEMhw3_5b.dat","w")) == NULL) {
  ...
  fprintf(rsp,"unset log x2\n");
  fprintf(rsp,"unset log y2\n");
  fprintf(rsp,"set ylabel 'Amplitude (A(w)) [m]'\n");
  fprintf(rsp,"set xlabel 'Frequency (w) [Hz]'\n");
  fprintf(rsp,"set title 'Amplitude vs Frequency'\n");
  fprintf(rsp,"set label 1 '{/=13 Amax = %em}'\nset label 1 at graph 0.50, 0.85 tc lt 3\n",Amax);
  fprintf(rsp,"set label 2 '{/=13 FWHM = %.1fm}'\nset label 2 at graph 0.50, 0.45 tc lt 3\n",(xr-xl));
  fprintf(rsp,"set label 3 '(FWHM = xr - xl = %em - %em)'\nset label 3 at graph 0.50, 0.41 tc lt 3\n",xr,xl);
