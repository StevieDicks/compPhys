/* Ex 8.8 R.O. Gray: Boundary value problem solved with the shooting method */
#include <stdio.h>
#include <math.h>
int flag = 0;
...
double integrate(double x1,double y1,double z1,double y11)
{
  ...
  while(x1 < 1.0 - dx/2.0) {
    k1 = z1*dx;
    l1 = (2.0*y1*y1*y1 + x1*y1)*dx;
    k2 = (z1+0.5*l1)*dx;
    l2 = (2.0*pow(y1+0.5*k1,3) + (x1+0.5*dx)*(y1+0.5*k1))*dx;
    k3 = (z1+0.5*l2)*dx;
    l3 = (2.0*pow(y1+0.5*k2,3) + (x1+0.5*dx)*(y1+0.5*k2))*dx;
    k4 = (z1+l3)*dx;
    l4 = (2.0*pow(y1+k3,3) + (x1+dx)*(y1+k3))*dx;
